...
#Start new loop for broken Game IDs
for (brokenGames in broken) {
  ...
  #Append to list
  brokenPBP[[brokenGames]] <- PBP
  
}

#Create data frame from broken games loop
brokenPBPDF <- do.call(rbind, brokenPBP)

#Merge to full data frame
fullpbpdf <- dplyr::bind_rows(bigdata, brokenPBPDF)

#Sort by gamecode then rownumber
fullpbpdf <- fullpbpdf[order(-gcode, -rownumber),]

#Cleaning
#build data frame of broken game IDs
n <- "broken"
df <- data.frame(broken, n)
df <- plyr::rename(df, c("broken" = "gcode"))

#merge list of broken games to full pbp file
fullpbpdf <- dplyr::left_join(fullpbpdf, df, by = "gcode")

#rename NA
fullpbpdf$json.missing <- ifelse(is.na(fullpbpdf$n), "good", "missing")

#New x,y cols, official
fullpbpdf$x.coord <- ifelse(fullpbpdf$json.missing == "good", fullpbpdf$x, fullpbpdf$xcoord)
fullpbpdf$y.coord <- ifelse(fullpbpdf$json.missing == "good", fullpbpdf$y, fullpbpdf$ycoord)

#Remove excess cols
fullpbpdf <- subset(fullpbpdf, select = -c(xcoord, ycoord, x, y, number, pos, lastfirst, last, first, numlast, numfirstlast, V1))

#write csv
#write.csv(fullpbpdf, file="1617pbp.csv")

#write RData
#save(fullpbpdf, file = "1617pbp.RData")